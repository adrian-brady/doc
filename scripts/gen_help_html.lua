...
local function visit_node(root, level, lang_tree, headings, opt, stats)
  ...
  elseif node_name == 'block' then
    ...
    if opt.old then
      -- Check if this block contains a help-tag-right (h4 pseudo-heading)
      if text:find('class="help%-tag%-right"') then
        -- Extract the tag anchor (up to and including <br>) and the content after
        local tag_part, rest = text:match('^(.-<br>)%s*(.*)$')
        if tag_part and rest then
          -- Split content at first tab or multiple spaces to separate term from description
          -- The content typically looks like: "zO		Open all folds..."
          local term, desc = rest:match('^([^%s\t]+)%s+(.+)$')
          if term and desc then
            return ('<div class="old-help-para help-tag-def">%s<div class="help-def-body"><span class="help-term">%s</span><span class="help-desc">%s</span></div></div>\n'):format(
              tag_part,
              term,
              desc
            )
      ...
      -- Check if this block starts with inline help-tag(s) followed by command definition
      -- Pattern: <span class="help-tag">...</span> [more tags] \n command		description
      if text:find('class="help%-tag"') and not text:find('class="help%-tag%-right"') then
        local tag_line, content = text:match('^(.-)%s*\n(.+)$')
        if tag_line and content and tag_line:find('class="help%-tag"') then
          -- Split content at first tab or multiple spaces to separate term from description
          local term, desc = content:match('^([^%s\t]+)%s+(.+)$')
          if term and desc then
            return ('<div class="old-help-para help-tag-def">%s<div class="help-def-body"><span class="help-term">%s</span><span class="help-desc">%s</span></div></div>\n'):format(
              tag_line,
              term,
